...
회귀는 현대 통계학을 이루는 큰 축이며 유전적 특성을 연구한느 영국의 통계학가 갈톤(Galton)이 수행한 연구에서 유래했다는 것이 일반론

"부모의 키가 크더라도 자식의 키가 대를 이어 무한정 커지지 않으며 , 부모의 키가 작더라도 대를 이어 자식의 키가 무한장 작아지지 않는다"

#### 회귀 개요

* 회귀는 여러 개의 독립변수와 한 개의 종속변수 간의 상관관계를 모델힝하는 기법을 통칭
* 결정값이 주어지는 지도학습
