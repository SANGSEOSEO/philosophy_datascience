...
대신 `PolynomialFeatures클래스`로 원본 단항 피처들을 다항 피처들로 변환한 데이터 세트에 `LinearRegression`객체를 적용하여 다항 회귀 기능을 제공함
...
즉, L1규제는 `alpha * ||W||_1` 를 의미하며, 라쏘 회귀 비용함수의 목표는 `RSS(W) + alpha * ||W||_1`식을 최소화하는 `W`를 찾는것.
...
* **엘라스틱넷의 규제는 `a L1규제 + b L2 규제`로 정의될 수 있습니다. **
...
| 변환 대상   | 설명                                                         |
| ----------- | ------------------------------------------------------------ |
| 타겟값 변환 | 회귀에서 타겟값은 반드시 정규 분포를 가져야 함. <br />이를 위해 주로 `로그 변환을 적용` |
...
|             | 2)스케일링/정규화를 수행한 데이터 세트에 다시 다항특성을 적용하여 변환하는 방법<br />보통 1)번 방법을 통해 예측 성능 향상이 없을 경우 적용할것을 고려함. |
|             | 원래 값에` log함수`를 적용하면 보다 정규 분포에 가까운 형태로 값이 분포된다.<br />로그변환은 매우 유용한 변환이며, 실제로 선형 회귀에서는 앞에서 소개한 1, 2번 방법보다 로그 변환이 훨씬 많이 사용되는 변환방법<br />그 이유는 , 1번 방법은 크게 예측 성능향상의 기대가 어려운 경우가 많고, 2번 방법의 경우 피처의 갯수가 매우 많은 경우엔 다항변환으로 생성되는 피처의 갯수가 기하급수적으로 늘어나 과적합이 발생할 수 있기 때문 |
...
단순 선형회귀 : `y = w1x + w0`가 있다고 가정할때 

로지스틱 회귀는 `0과 1`을 예측하기에 단순 회귀식은 큰 의미가 없습니다. 
...
* 사이킷런의 결정 트리 및 결정 트리 기반의 양상블 알고리즘은 분류 뿐만 아니라 회귀도 가능합니다. 이는 트리가 `CART(Classification and Regression Tree)`를 기반으로 만들어 졌기 때문입니다. `CART`는 분류 뿐만 아니라 회귀도 가능한 트리 분할 알고리즘입니다.
* `CART`회귀 트리는 분류와 유사하게 분할을 하며, 분할 기준은 RSS(SSE)가 최소가 될 수 있는 기준을 찾아서 분할합니다.
...
![datasets](https://user-images.githubusercontent.com/70785000/123938991-a0db0580-d9d2-11eb-9184-c9f9a4fc6454.PNG)

위의 데이터셋을 

* 1) `RSS`를 최소화하는 규칙 기준에 따라 분할

  루트 노드를 `split 0` 기준으로 분할하고, 이렇게 분할된 규칙노드에서 다시 `Split 1`과 `Split 2` 규칙노드로 분할할 수 있습니다. 그리고 `Split 2`는 다시 재귀적으로 `Split 3 `규칙 노드로 아래 그림의 오른쪽과 같이 트리 규칙으로 변환.

![회귀트리1](https://user-images.githubusercontent.com/70785000/123939186-d4b62b00-d9d2-11eb-8431-495089008342.PNG)

* 2) 최종 분할된 영역에 있는 데이터들의 평균값들로 학습 / 예측

  리프 노드 생성 기준에 부합하는 트리 분할이 완료되었다면 리프 노드에 소속된 데이터 값의 `평균값`을 구해서 최종적으로 리프 노드에 결정값으로 할당

![회귀트리2](https://user-images.githubusercontent.com/70785000/123963363-574be400-d9ed-11eb-9e11-678e1a643378.PNG)
...
| `Gradient Boosting` | `GradientBoostingRegressor` | `GradientBoostingClassifer` |
| `XGBoost`           | `XGBRegressor`              | `XGBClassifier`             |
| `LightGBM`          | `LGBMRegressor`             | `GBMClassifier`             |

##### 회귀트리의 오버 피팅

회귀트리 역시 복잡한 트리 구조를 가질 경우 오버 피팅하기 쉬우므로 트리의 크기와 노드 갯수의 제한 등의 방법을 통해 오버 피팅을 개선할 수 있습니다.

​                                              		<`max_depth`가 개수에 따라 오버피팅>

![regressor_overfitting](https://user-images.githubusercontent.com/70785000/124038782-ad944380-da3c-11eb-84fe-933f07583fe1.PNG)

#### 고급회귀 기법

![고급회귀기법](https://user-images.githubusercontent.com/70785000/124141922-c3e5e200-dac4-11eb-8633-24651bde84ee.PNG)

#### 정리

* 선형회귀와 비용함수 RSS

  독립변수(피처), 종속변수(타겟)의 관계를 선형으로 예측해서 선형함수를 찾는것.결국은 `기울기- (w1), 절편(w0)- 이 회귀계수를 찾아주는것인데,회귀계수를 찾기위해 RSS(Residual Sum of Squares- 실제값과 예측값의 차이의 제곱값)`를 비용함수로  설정했고, 이 비용함수가 최소인 w값을 찾기위해서 경사하강법을 통해 비용을 계속 감소시키는 방향성으로 가지고 회귀계수값을 업데이트하면서 최소비용을 갖는 회귀계수를 찾아감

* 다항회귀와 과소적합/과대적합

  높은 `degree`로 갈수록 학습데이터에만 `fit`되어 과대적합되어 비용함수만 줄여주는 결과가 나온다. 그래서 이에 대한 대안으로 규제가 등장하게 됨.
...
* 왜곡도 개선을 위한 데이터변환과 원-핫인코딩

* 실습 예제를 통한 데이터 정제와 변환 그리고 선형 회귀/회귀트리/혼합모델/스태킹모델 학습/ 예측 /평가 비교
